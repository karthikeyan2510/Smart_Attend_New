    ...
    @objc open func highlightValue(_ highlight: Highlight?, callDelegate: Bool)
    {
        ...
        if callDelegate, let delegate = delegate
        {
            if let h = h
            {
                ...
                //We Modified
                let dict=["highlight.x":h.x,"highlight.y":h.y,"entry.x":entry!.x,"entry.y":entry!.y] as [String: Double]
                NotificationCenter.default.post(name: NSNotification.Name(rawValue: "Linechart_Selectvalue"), object: nil, userInfo: dict)
            }
            else
            {
